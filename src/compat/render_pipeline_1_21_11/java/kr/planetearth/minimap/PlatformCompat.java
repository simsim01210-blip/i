...
final class PlatformCompat {
    ...
    static void push(DrawContext context) { context.getMatrices().pushMatrix(); }
    static void pop(DrawContext context) { context.getMatrices().popMatrix(); }
    ...
    static PositionedSoundInstance controlSound(float pitch, float volume) {
        return PositionedSoundInstance.ui(SoundEvents.ENTITY_GENERIC_EAT.value(), pitch, volume);
    }

    static PositionedSoundInstance openCloseSound(float pitch, float volume) {
        return PositionedSoundInstance.ui(
    ...
    static PositionedSoundInstance navigationCompleteSound(float pitch, float volume) {
        return PositionedSoundInstance.ui(SoundEvents.ENTITY_PLAYER_LEVELUP, pitch, volume);
    }
}
